...
function ProdutosListaPage({ atualizarTotal, addProdutoCarrinho }) {
  ...
  function deveExibirMensagem() {
    return !isLoading && message?.trim() && messageType == 1;
  }

  function deveExibirMensagemNenhumProduto() {
    return !isLoading && produtos?.length == 0;
  }
  ...
  return (
    <>
      ...
      {deveExibirMensagem() && <Message type={messageType} message={message} />}
      {deveExibirMensagemNenhumProduto() && (
        <Message type={0} message="Nenhum Produto Encontrado!" />
      )}
      {/* {!isLoading && produtos?.length > 0 && construirElementos()} */}
      ...
      {[{ nome: "nikola" }, { nome: "alan" }].map((obj) => {
        return <p style={{ fontSize: "2em"}}>{obj.nome}</p>;
      })} */}
    </>
  );
